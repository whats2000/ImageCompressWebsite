...
from utils.webp_compression import webp_compression

def test_webp_compression_basic():
    """
    Test basic functionality of WebP compression
    """
    # Create a test image (a simple small image)
    test_image = Image.new('RGB', (100, 100), color='red')
    
    # Compress the image
    compressed_image = webp_compression(test_image)
...
def test_webp_compression_quality_levels():
    """
    ...
    for quality in quality_levels:
        compressed_image = webp_compression(test_image, quality)
...
def test_webp_compression_color_preservation():
    """
    ...
    # Compress the image
    compressed_image = webp_compression(test_image, quality=85)
...
def test_webp_compression_invalid_input():
    """
    Test error handling for invalid inputs
    """
    # Test with None
    with pytest.raises(ValueError, match="Input image cannot be None"):
        webp_compression(None)
    
    # Test with invalid image type
    with pytest.raises(TypeError, match="Expected PIL Image"):
        webp_compression("not an image")
    
    # Test with invalid quality values
    with pytest.raises(ValueError, match="Quality must be between 1 and 100"):
        webp_compression(Image.new('RGB', (50, 50), color='green'), quality=0)
    
    with pytest.raises(ValueError, match="Quality must be between 1 and 100"):
        webp_compression(Image.new('RGB', (50, 50), color='green'), quality=101)
    
    with pytest.raises(TypeError, match="Quality must be an integer"):
        webp_compression(Image.new('RGB', (50, 50), color='green'), quality="not an int")

def test_webp_compression_memory_efficiency():
    """
    ...
    # Compress the image
    compressed_image = webp_compression(test_image, quality=50)
